...
product_details AS (
    ...
    FROM STAGING_SCHEMA.PRODUCT_DETAILS
),
product_reviews AS (
    SELECT
        -- Product reviews
        ...
        review_rating,
        review_comment
    FROM STAGING_SCHEMA.PRODUCT_REVIEWS
)
...
WITH customer_purchases AS (
    ...
    FROM STAGING_SCHEMA.customers AS c
    JOIN STAGING_SCHEMA.purchases AS p ON c.customer_id = p.customer_id
    ...
    SELECT
        p.product_id,
        c.category_name
    FROM STAGING_SCHEMA.products AS p
    JOIN STAGING_SCHEMA.categories AS c ON p.category_id = c.category_id
), product_reviews AS (
    SELECT
        pr.product_id,
        pr.reviewer_id,
    ...
    FROM STAGING_SCHEMA.product_reviews AS pr
